...
open class Chartlet: CanvasElement {
    ...
    public var shownRect: NSRect = NSRect(x: 0, y: 0, width: 1, height: 1)
    ...
    init(center: CGPoint, size: CGSize, textureID: String, angle: CGFloat, canvas: Canvas, shownRect: NSRect = NSRect(x: 0, y: 0, width: 1, height: 1)) {
    ...
    lazy var vertex_buffer: MTLBuffer? = {
        ...
        let vertexes = [
            Vertex(position: CGPoint(x: center.x - halfSize.width, y: center.y - halfSize.height).rotatedBy(angle, anchor: center),
                   textCoord: CGPoint(x: self.shownRect.minX, y: self.shownRect.minY)),
            Vertex(position: CGPoint(x: center.x + halfSize.width , y: center.y - halfSize.height).rotatedBy(angle, anchor: center),
                   textCoord: CGPoint(x: self.shownRect.maxX, y: self.shownRect.minY)),
            Vertex(position: CGPoint(x: center.x - halfSize.width , y: center.y + halfSize.height).rotatedBy(angle, anchor: center),
                   textCoord: CGPoint(x: self.shownRect.minX, y: self.shownRect.maxY)),
            Vertex(position: CGPoint(x: center.x + halfSize.width , y: center.y + halfSize.height).rotatedBy(angle, anchor: center),
                   textCoord: CGPoint(x: self.shownRect.maxX, y: self.shownRect.maxY)),
        ]
